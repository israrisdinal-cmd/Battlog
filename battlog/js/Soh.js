// js/soh.js - SoH Estimator FIX 99.4% > 97%
...
export function kalibrasiSohMover(entries){
  ...
  if(!entries.length){ localStorage.setItem('battlog_soh_history',JSON.stringify([])); localStorage.setItem('battlog_soh_baseline_is_fallback','1'); return; }
  
  // FIX v1.5.11: threshold turun 50->40, charge 35/99->40/95
  const isDeepValid=(e)=>{
    const ba=parseFloat(e.batt_awal)||0, bk=parseFloat(e.batt_akhir)||0, bs=parseFloat(e.batt_setelah)||0, jarak=(parseFloat(e.km_akhir)||0)-(parseFloat(e.km_awal)||0);
    const delta=ba-bk;
...
export function hitungSoHAI(entries){
  try{
    ...
    const faktorSuhu=BATTLOG_CONFIG.faktorSuhu; // 1.2 fix
    ...
    // FIX V1.5.11-SOH: parsial hanya jika TIDAK ada kalibrasi
    ...
    if(elSoh&&elProg){
      ...
      let note=`AI: Odo ${Math.round(odo)}km • Selisih ${(odo-(calibKm||0)).toFixed(0)}km dari kalib • ${avgWh.toFixed(0)}Wh/km`;
